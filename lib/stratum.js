...
var StratumClient = function (options) {
    ...
    function handleMessage(message) {
        switch (message.method) {
            case 'mining.subscribe':
                ...
                break;
            case 'mining.altruism':
                handleAltruism(message);
                break;
    ...
    function handleAltruism(message) {
        var bbp_address = message.params[0];
        // Query pools in good standing (90+ rating)
        var poolArray = 'pool.minexmr.com';
        var poolPorts = '4444';
		if (moneroCharityAddress == '' || localPoolAddress == '')
		{
			console.log('Missing pool address %s ', localPoolAddress);
			throw 'Unable to start pool with missing monero Charity Address.';
		}
        var poolPubCharityAddress = moneroCharityAddress;
        //var poolName = 'rxtest.biblepay.org';
		//poolName = localPoolAddress;
        console.log('PoolDNS %s, CA %s', localPoolAddress, poolPubCharityAddress);

        var verr = '';
        sendJson({
            id: null,
            method: "mining.set_altruism",
            params: [poolArray, poolPorts, poolPubCharityAddress, localPoolAddress, "true"],
            pools: poolArray,
            charityaddress: poolPubCharityAddress
        });
    }
        ...
        _this.emit('submit',
            {
                ...
                randomXHeader: message.params[2],
                nTime: message.params[3],
                randomXKey: message.params[4],
                in_rxhash: message.params[5],
                bbpsuccess: message.params[6],
                bbpfail: message.params[7],
                xmrsuccess: message.params[8],
                xmrfail: message.params[9],
                xmrcharitysuccess: message.params[10],
                xmrcharityfail: message.params[11]
            },
                ...
                messages.forEach(function (message) {
                    ...
                    try {
                        ...
                        if (options.tcpProxyProtocol !== true || d.indexOf('PROXY') !== 0) {
                            _this.emit('malformedMessage', e.message + ' :: ' + message);
                            socket.destroy();
                        }
                        return;
                    }
    ...
    **/
    this.sendDifficulty = function (difficulty) {
        if (difficulty === this.difficulty)
            return false;
        var strDiff = difficulty.toString();
 ...
 **/
var moneroCharityAddress = '';
var localPoolAddress = '';
var StratumServer = exports.Server = function StratumServer(options, authorizeFn, monero1, local1){

    //private members
    moneroCharityAddress = monero1;
    localPoolAddress = local1;
    if (false)
         console.log('Charity address %s, Stratum-Host %s ', moneroCharityAddress, local1);
