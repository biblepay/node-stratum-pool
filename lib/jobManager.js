...
var JobManager = module.exports = function JobManager(options) {
    ...
    this.processShare = function (jobId, previousDifficulty, difficulty, nTime, ipAddress,
     port, workerName, out_rx, rxData, out_rxroot, poolAddress, rxSeed, blockSolution, in_rxhash) {
        ...
        if (nTime < submitTime - (60 * 7) || nTime > submitTime + 7200 || nTime == 0) {
            if (true) {
                console.log('ntime out of range nTime %d, rpctime %d, rxdata %s', nTime, job.rpcData.curtime, rxdata);
        ...
        if (rx_hash_analyzed != out_rx) {
            console.log('stale share rx_hash %s, [%s] solving for wrong height?', out_rx, rx_hash_analyzed);
        ...
        if (job.target.ge(headerBigNum)) {
            ...
            console.log('***** WE SOLVED THE BLOCK at height %s ******!!! ', rxData);
            blockSolved = 1;
        }
        else {
            blockSolved = 0;
        ...
        if (false)
            console.log('\n*****************  Block solved %d , block solve data %s ',blockSolved,  blockSolution);
